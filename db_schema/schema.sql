...
--  PostgreSQL Schema — rebuilt to match Forensic_Medico_Legal_Database_ER.pdf
--  Sections: Security & Admin | People & Organisations |
--            Case Master Log | Case Events & Medical Subtypes |
--            Specimens & Laboratory Chain
-- ============================================================
...
-- M:N resolution: staff assigned to a case
...
--  (CASE_EVENT is the supertype; CLINICAL_EXAMINATION,
--   POSTMORTEM and SKELETAL_EXAMINATION are mutually-exclusive
--   subtypes sharing the same PK/FK = event_id)
-- ============================================================
...
-- Lookup: Grievous hurt clauses (Penal Code s.311 (a)-(i))
...
-- Optional extension of a clinical examination
...
SELECT
    cm.case_id, cm.case_number, cm.inquest_no, cm.case_type, cm.status,
    cm.incident_date, cm.received_date,
...
SELECT r.report_id, r.serial_no, r.report_type, r.status, r.date_issued,
       cm.case_number, s.full_name AS prepared_by
FROM report r
...
SELECT ce.event_id, ce.case_id, ce.event_type, ce.event_date,
       s.full_name AS conducted_by, cm.case_number
FROM case_event ce
